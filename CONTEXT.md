...
**Today**:
The pinned, cross-List view of what is due — the first of the sidebar's two pinned rows (the **Weekly spread** is the second), always selectable, never a real List. Membership is `due <= today` (`domain::due_on_or_before`, the one definition, shared by the cache aggregate and the view filter); an **undated** entry is therefore never in it. A Completed row shows only if it was completed today (`domain::within_completion_day`, the one definition, shared with the JSON CLI's `today` so the pane and a bar can never disagree about which completions belong to the day), so the pane answers "among what was due, what got done". Flat and read-only in ordering terms: no Subtask nesting, no Manual lens (`position` is per-List, so a cross-List hand order is undefined), and a Manual lens carried in from a List is normalised to Due on entry. Renders as a **Journal spread**.
